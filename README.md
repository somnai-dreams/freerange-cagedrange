...
The proof calculation is available independently of JSX through Freerange's layout algebra. Constants, named symbolic measurements, addition, scaling, `min`, `max`, alternatives, row/column block composition, border-box composition, and logical box metrics produce normalized layout expressions. `proveLayoutEquality(left, right, tolerance)` returns `proven`, `violated`, or `unknown`. Identical symbolic terms cancel, so two starts derived from the same unknown header height can still be proven equal; unrelated unknown heights remain unknown. Source formats such as JSX and Tailwind are frontends for this algebra, not part of its proof semantics.
