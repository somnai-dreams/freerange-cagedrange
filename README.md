...
A computed `className` is read as far as it goes statically: string parts of cn/clsx/twMerge-style calls (matched by lowercased name, so a project wrapper like `CN` counts), object keys, template literals, ternary and `&&`/`||` branches. The visible classes go through the same checks — a conditional `mt-2` conflicting with an inline margin is still mixture — and a token fused with a dynamic part, e.g. `` `mt-${size}` ``, is dropped rather than guessed. When parts stay unseen, the element cannot be proven unpositioned, and a note marks that unseen classes exist. Fully opaque constructs — a props spread, a spread inside the style object — still report a single unscannable note. Library users can call `scanSpacingSource(file, source)` for the same findings as structured data.
